...
def training_stardist(image_path,save_path,save_name, folder_list):
    # path = "/home/nakajima/work/Ecoli/code/stardist_local/mymodel/2D_versatile_fluo_256_traincolor/stardist_trained_model.h5"
    # path = "/home/nakajima/work/Ecoli/code/stardist_local/models/paper/2D_dsb2018/weights_last.h5"
    ...
    model = StarDist2D(config, name=save_name, basedir=save_path)
    ...
    x_test, y_test = load_images_and_labels(test_dir,False)
...
#train
    ...
    model.keras_model.save("savedmodel")
    # model.keras_model.save(os.path.join(save_path,"stardist_trained_model.h5"))
    # model.export_TF()

    # # 予測
    val_save_pred_path = os.path.join(save_path,'val_sem_image')
    val_save_sem_pred_path = os.path.join(save_path,'val_image')
    val_save_pred_nor_path = os.path.join(save_path,'val_normalize_image')
    os.makedirs(val_save_pred_path,exist_ok=True)
    os.makedirs(val_save_sem_pred_path, exist_ok=True)
    os.makedirs(val_save_pred_nor_path,exist_ok=True)
    for i, test_img in enumerate(x_val):
        instance = model.predict_instances(normalize(test_img))
        iou, non_nan_iou =  f1_iou(instance[0],y_val[i],len(np.unique(y_test[i])))
        semantic = np.where(instance[0]>0, 1, 0)
        # print(np.unique(semantic))
        label = np.where(y_test[i].astype(np.uint8), 1, 0)
        sem_iou,  sem_non_nan_iou =  f1_iou(semantic,label,2)
        semantic = np.where(semantic>0, 255, 0)
        ...
        cv2.imwrite(os.path.join(val_save_sem_pred_path,f'{i + 1:05d}.png'), semantic)
        cv2.imwrite(os.path.join(val_save_pred_path,f'{i + 1:05d}.png'), instance[0])
        cv2.imwrite(os.path.join(val_save_pred_nor_path,f'{i + 1:05d}.png'),  (instance[0] * (255/(np.max(instance[0])+1))).astype(np.uint8))
...
def predict(image_path,save_path,folder_list):
    # 保存済みのモデルを読み込む
    # path = "/home/nakajima/work/Ecoli/code/stardist_local/mymodel/pretrain_2D_versatile_fluo_256/2025-07-19-20-03-21.400076/stardist_trained_model.h5"
    # path = "/home/nakajima/work/Ecoli/code/stardist_local/mymodel/pretrain_2D_versatile_fluo_256/20250720141734/TF_SavedModel"
    name = 'metrics5.csv'
    model_pretrained = StarDist2D.from_pretrained('2D_versatile_fluo')
    # model_pretrained = StarDist2D.from_pretrained('2D_paper_dsb2018')
    ...
    save_pred_path = os.path.join(save_path,'pred_image')
    save_pred_nor_path = os.path.join(save_path,'pred_normalize_image')
    save_pred_sem_path = os.path.join(save_path,'pred_sem_image')
    os.makedirs(save_pred_path,exist_ok=True)
    os.makedirs(save_pred_nor_path,exist_ok=True)
    os.makedirs(save_pred_sem_path, exist_ok=True)
    # model = StarDist2D(None, name='a', basedir=save_path)
    # model.load_weights(path)
    ...
    # img = x_test[32]
    # labels,_ = model_pretrained.predict_instances(normalize(img))
    # plt.subplot(1,3,1)
    ...
    all_rows = []
    for i, test_img in enumerate(x_test):
        instance,_ = model_pretrained.predict_instances(
            img = normalize(test_img), 
        )
        # metrics =  matching(test_label, instance[0])
        # parse_and_write_metrics(save_path+'/'+name, str(metrics))
        cv2.imwrite(os.path.join(save_pred_path,f'{i + 1:05d}.png'), instance)
        if np.max(instance) == 0:
            cv2.imwrite(os.path.join(save_pred_nor_path, f'{i + 1:05d}.png'),  instance)
        else:
            cv2.imwrite(os.path.join(save_pred_nor_path, f'{i + 1:05d}.png'),  (instance * (255//(np.max(instance)))))
        
        semantic = np.where(instance>0, 1, 0)
        labels = np.where(y_test[i]>0, 1, 0)
        sem_iou,  sem_non_nan_iou =  f1_iou(semantic,labels,2)
        row = np.concatenate([sem_iou, sem_non_nan_iou])
        all_rows.append(row)
        # save_score([sem_iou,  sem_non_nan_iou],os.path.join(save_path,f'semantic_{name}'))
        instance = np.where(instance>0, 255, 0)
        cv2.imwrite(os.path.join(save_pred_sem_path,f'{i + 1:05d}.png'), instance)
    with open(os.path.join(save_path,f'semantic_{name}'), 'w', newline='') as f:
        writer = csv.writer(f)
        for row in all_rows:
            writer.writerow(row)
    ...
    stat_section1 = {
    'mean': np.nanmean(data[:, 0:4], axis=0),
    'variance': np.nanvar(data[:, 0:4], axis=0),
    'std_dev': np.nanstd(data[:, 0:4], axis=0)
    }

    adjusted_data = data[:, 4:].astype(float)
    adjusted_data[adjusted_data == 0] = np.nan

    stat_section2 = {
        'mean': np.nanmean(adjusted_data, axis=0),
        'variance': np.nanvar(adjusted_data, axis=0),
        'std_dev': np.nanstd(adjusted_data, axis=0)
    }

    with open(os.path.join(save_path,f'semantic_{name}'), 'a', newline='') as f:
        writer = csv.writer(f)
        writer.writerow(['mean'] + list(stat_section1['mean']) + list(stat_section2['mean']))
        writer.writerow(['variance'] + list(stat_section1['variance']) + list(stat_section2['variance']))
        writer.writerow(['std_dev'] + list(stat_section1['std_dev']) + list(stat_section2['std_dev']))
    # モデルの保存
...
def main():
    path =  '/home/nakajima/work/Ecoli/code/Unet/data/64_16'
    savepath1 = '/home/nakajima/work/Ecoli/code/stardist_local/mymodel/pretrain_2D_versatile_fluo_64'
    savepath2 = '/home/nakajima/work/Ecoli/code/stardist_local/mymodel/2D_versatile_fluo_256_traincolor'
    ...
    # training_stardist(path,savepath1,save_name, folder_list[0])
    predict(path,savepath1, folder_list[0])
